namespace DbMon.NET {
	...
	public sealed class DebugMonitor {
		...
		/// </summary>
		public event OnOutputDebugStringHandler OnOutputDebugString;		

		/// <summary>
		/// Event handle for slot 'DBWIN_BUFFER_READY'
		/// </summary>
		private IntPtr m_AckEvent = IntPtr.Zero;

		/// <summary>
		/// Event handle for slot 'DBWIN_DATA_READY'
		/// </summary>
		private IntPtr m_ReadyEvent = IntPtr.Zero;

		/// <summary>
		/// Handle for our shared file
		/// </summary>
		private IntPtr m_SharedFile = IntPtr.Zero;

		/// <summary>
		/// Handle for our shared memory
		/// </summary>
		private IntPtr m_SharedMem = IntPtr.Zero;

		/// <summary>
		/// Our capturing thread
		/// </summary>
		private Thread m_Capturer = null;
        ...
        /// <summary>
        /// Scope prefix for synchronization objects
        /// </summary>
	    private string m_Scope;

	    public DebugMonitor(bool globalScope)
        {
            m_Scope = globalScope ? @"Global\" : @"Local\";
        }
		...
		public void Start() {
			lock (m_SyncRoot) {
				...
				m_Mutex = new Mutex(false, m_Scope + typeof(DebugMonitor).Namespace, out createdNew);				
				if (!createdNew)
				...
				m_AckEvent = CreateEvent(ref sa, false, false, m_Scope + "DBWIN_BUFFER_READY");
				...
				m_ReadyEvent = CreateEvent(ref sa, false, false, m_Scope + "DBWIN_DATA_READY");
				...
				m_SharedFile = CreateFileMapping(new IntPtr(-1), ref sa, PageProtection.ReadWrite, 0, 4096, m_Scope + "DBWIN_BUFFER");
		...
		private void FireOnOutputDebugString(int pid, string text) {
		...
		private ApplicationException CreateApplicationException(string text) {
			...
			return new ApplicationException(
                string.Format("{0}. Last Win32 Error was {1}", text, Marshal.GetLastWin32Error()));
		}
